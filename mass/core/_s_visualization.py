...
def plot_simulation(time, solution_profile, default_fontsize=15, **kwargs):
    ...
    time: tuple, list, or numpy.ndarray
        A tuple of start and end points or a list or raw endpoints
    solution_profile : dict of scipy.interp1d
        An dict of interpolating functions containing the simulated results 
        for either concentration or flux
...
def _gen_np_time_vector(time, **options):
    # Ensure time is tuple or list/np.ndarray of time points
    time_type = None
    if isinstance(time, tuple):
        time_type = "tuple"
    elif isinstance(time, list) or isinstance(time, np.ndarray):
        time_type = "list/np.ndarray"
    else:
        msg = "time must be of type tuple, list, or numpy.ndarray"
        raise TypeError(msg)

    # if time is tuple: use plot_function to generate linspace or geomspace
    log_xscale, log_yscale = _is_log_scale(default=True, **options)
    if (time_type is "tuple") and log_xscale:
        if time[0] == 0:
            warn("start point cannot be 0, using 1e-6 instead")
            time = list(time)
            time[0] = 1e-6
            time = tuple(time)
            time = np.geomspace(*time)
    elif (time_type is "tuple") and (not log_xscale):
            time = np.linspace(*time)

    # elif time is list or np.npdarray: use points as is
    elif time_type is "list/np.ndarray":
        pass
