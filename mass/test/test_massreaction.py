...
class test_MassReaction(unittest.TestCase, MassReaction):
	...
	def setUp(self):
		...
		## basic react for testing
		self.react1 = MassReaction("TestIDr1")
		## react with irreversibility
		self.react2 = MassReaction("TestIDr2", reversibility=False)

		# subsystem and other relevents
		## react w/subsystem
		self.react3 = MassReaction("TestIDr3",
									subsystem = "Techoic Acid Biosynthesis")
		## react w/different subsystem
		self.react4 = MassReaction("TestIDr4",
	...
	def test_IsInstance_from_cobra_reaction(self):
		cmr = MassReaction.from_cobra_reaction(CobraReaction=self.cr,
												mass_id=self.cr.id)
	...
	def test_get_compartments(self):
		...
		self.assertEqual(self.react1.get_compartments(),
						list(self.react1.compartments))
		self.assertNotEqual(self.react1.get_compartments(),
							self.react1.compartments)
	...
	def test_get_coefficient(self):
		...
		expected_value = -1
		self.assertEqual(self.react1.get_coefficient(metab6),
						expected_value)
		self.assertNotEqual(self.react1.get_coefficient(metab7),
							expected_value)

		expected_value = 2
		self.assertNotEqual(self.react1.get_coefficient(metab6),
							expected_value)
	...
	def test_get_coefficients(self):
		...
		expected_value = [-1, 2]
		self.assertEqual(list(self.react1.get_coefficients([metab6, metab7])),
							expected_value)
	...
	def test_name_not_string_has_typeerror_int(self):
		with self.assertRaisesRegex(TypeError,
									"name must be a string type") as t:
	...
	def test_subsystem_not_string_has_typeerror_int(self):
		with self.assertRaisesRegex(TypeError,
									"subsystem must be a string type") as t:
	...
	def test_reversibility_not_bool_has_typeerror_int(self):
		with self.assertRaisesRegex(TypeError,
									"reversibility must be a boolean") as t:
	...
	def test_reversibility_not_bool_has_typeerror_float(self):
		with self.assertRaisesRegex(TypeError,
									"reversibility must be a boolean") as t:
			self.react_temp = MassReaction(id="TestIDr_temp",
											reversibility=4.5)
			del self.react_temp
